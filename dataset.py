...
def create_dataset(Name):
    print(f"Starting face dataset creation for: {Name}")
    ...
    try:
        with tf.Graph().as_default():
            with tf.compat.v1.Session() as sess:
                ...
                print("Camera opened successfully. Press 's' to capture face, 'q' to quit")
                ...
                while True:
                    ...
                    if len(face_patches) > 0:
                        ...
                        for bb in padded_bounding_boxes:
                            cv2.rectangle(display_frame, (bb[0], bb[1]), (bb[2], bb[3]), (0, 255, 0), 2)
                        
                        # Display instruction
                        cv2.putText(display_frame, f"Faces detected: {len(face_patches)}", 
                                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                        cv2.putText(display_frame, "Press 's' to capture face", 
                                   (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                        cv2.putText(display_frame, "Press 'q' to finish", 
                                   (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
                        cv2.putText(display_frame, f"Captured: {capture_count}/{max_captures}", 
                                   (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
                    else:
                        cv2.putText(display_frame, "No face detected", 
                                   (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
                        cv2.putText(display_frame, "Position face in camera view", 
                                   (10, 60), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)

                    cv2.imshow("Face Registration - Press 's' to capture, 'q' to quit", display_frame)
                    
                    key = cv2.waitKey(1) & 0xFF
                    
                    if key == ord('s') and len(face_patches) > 0:
                        # Capture the face
                        for emb, bb in zip(embs, padded_bounding_boxes):
                            patch = frame[bb[1]:bb[3], bb[0]:bb[2], :]
                            success = id_data.add_id(emb, Name, patch)
                            if success:
                                capture_count += 1
                                print(f"Face sample {capture_count} captured for {Name}")
                                face_captured = True
                                
                                # Show confirmation
                                confirmation_frame = frame.copy()
                                cv2.putText(confirmation_frame, f"CAPTURED! {capture_count}/{max_captures}", 
                                           (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                                cv2.imshow("Face Registration - Press 's' to capture, 'q' to quit", confirmation_frame)
                                cv2.waitKey(500)  # Show confirmation for 500ms
                        
                        if capture_count >= max_captures:
                            print(f"Successfully captured {max_captures} face samples for {Name}")
                            break
                ...
                if capture_count > 0:
                    print(f"Successfully captured {capture_count} face samples for {Name}")
                    return True
                else:
...
##    name = input("Enter name for face registration: ")
##    result = create_dataset(name)
